...
class WS2812:
    ...
    def update_buf(self, data, start=0):
        ...
        mask = 0x03
        index = start * 16
        for red, green, blue, white in data:
            red = int(red * intensity)
            green = int(green * intensity)
            blue = int(blue * intensity)
            white = int(white * intensity)
            ...
            buf[index+12] = buf_bytes[white >> 6 & mask]
            buf[index+13] = buf_bytes[white >> 4 & mask]
            buf[index+14] = buf_bytes[white >> 2 & mask]
            buf[index+15] = buf_bytes[white & mask]
    ...
    def fill_buf(self, data):
        ...
        for index in range(end * 16, self.buf_length):
            buf[index] = off
            index += 1
